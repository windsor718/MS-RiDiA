...
export IFORTLIB="/share/pkg/intel/parallel_studio_xe_2018_u3ce/lib:/share/pkg/intel/parallel_studio_xe_2018_u3ce/mkl/lib"
...
##EXP="MSR_03min"                       # experiment name (output directory name)
...
LRIVSHP=".TRUE."                            # .TRUE. to activate river cross section geometry


#============================
#*** 1c. simulation time
##YSTA=1984                                   # start year ( from YSTA / Jan  1st _ 00:00)
##YEND=2010                                   # end   year (until YEND / Dec 31st _ 24:00)
...
##CROFDIR="${BASE}/inp/MSR_15min/vic/"     # runoff directory
CROFPRE="Roff_15min_MSR"                          # runoff prefix/suffix  
...
FMAP="${BASE}/map/MSR_03min"                # map directory
CDIMINFO="${FMAP}/diminfo_VIC-15min.txt"    # dimention information file
CINPMAT=${FMAP}/inpmat_VIC-15min.bin        # runoff input matrix for interporlation
...
##CRIVMAN="${FMAP}/rivman_adcp.bin"           # manning coefficient river (The one in flood plain is a global parameter; set $PMANFLD below.)
##CRIVSHP="${FMAP}/rivshp.bin"                # river cross section geometry parameter
##CRIVBTA="${FMAP}/rivbta.bin"                # river wetted perimeter parameter
...
do 
  ...
  if [ ${SPINUP} -eq 0 ];then
    ...
    LRESTART=".TRUE."
    CRESTSTO="${CVNREST}.bin"    ## from restart file
...
LRIVSHP  = ${LRIVSHP}                  ! true: activate river cross section geometry
/
&NDIMTIME
...
CRIVSHP    = "${CRIVSHP}"              ! river cross section geometry parameter
CRIVBTA    = "${CRIVBTA}"              ! river wetted perimeter parameter
